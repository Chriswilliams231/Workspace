 <header class="bg-blue-900 text-white p-4" x-data="{ open: false }">
     <div class="container mx-auto flex justify-between items-center">
         ...
         <nav class="hidden md:flex items-center space-x-4">
             <!-- Componet Header that uses props for the navigation -->
             <x-nav-link url="/" :active="request()->is('/')">Home</x-nav-link>
             <x-nav-link url="/jobs" :active="request()->is('jobs')">All Jobs</x-nav-link>
             @auth
                 <x-nav-link url="/bookmarks" :active="request()->is('bookmarks')">Save Jobs</x-nav-link>
                 ...
                 <div class="flex items-center space-x-3">
                     <a href="{{ route('dashboard') }}">
                         ...
                         @else
                             <img src="{{ asset('storage/avatars/test_avatar.png') }}" alt="{{ Auth::user()->name }}"
                                 class="w-10 h-10 rounded-full">
                         @endif
                     </a>
                 </div>
         ...
         <button x-on:click="open = ! open" id="hamburger" class="text-white md:hidden flex items-center">
     ...
     <nav x-show="open" @click.outside="open = false" id="mobile-menu"
         ...
         @auth
             <x-nav-link url="/bookmarks" :active="request()->is('bookmarks')" :mobile="true">Save Jobs</x-nav-link>
