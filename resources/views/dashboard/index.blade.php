    ...
    <section class="flex flex-col md:flex-row gap-4">
        ...
        <div class="bg-white p-8 rounded-lg shadow-md w-full">
            ...
            <form action="{{ route('profile.update') }}" method="POST" enctype="multipart/form-data">
                ...
                <button type="submit"
                    class="w-full bg-green-500 hover:bg-green-600 text-white px-4 py-2 focus:outline-none cursor-pointer">Submit</button>
            </form>
        </div>
        {{-- Job listing section --}}
        <div class="bg-white p-8 rounded-lg shadow-md w-full">
            ...
            @forelse ($jobs as $job)
                <div class="flex justify-between items-center border-b-2 border-gray-200 py-2">
                    ...
                    <div class="flex space-x-3">
                        ...
                        <form method="POST" action="{{ route('jobs.destroy', $job->id) }}?from=dashboard"
                            onsubmit="confirm('Please confirm you would like to delete this job')">
                            @csrf
                            @method('DELETE')
                            <button type="submit"
                ...
                <div class="mt-4 bg-gray-100 p-2">
                    <h4 class="text-lg font-semibold mb-2">Applicants who applied</h4>
                    @forelse ($job->applicants as $applicant)
                        <div class="py-2">
                            ...
                            {{-- Delete Applicant --}}
                            <form method="POST" action="{{ route('applicant.destory', $applicant->id) }}"
                                onsubmit="return confirm('Confirm you want to delete this applicant.')">
                                @csrf
                                @method('DELETE')
                    ...
                    @empty
                        <p class="text-gray-700">No applicants have applied</p>
                    @endforelse

                </div>
            @empty
                <p class="text-gray-700">You have no listings at this time</p>
            @endforelse
        </div>
    </section>
    <x-bottom-banner />
